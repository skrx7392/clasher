...
- **FR-18** The system shall capture and persist regular-war and CWL data while it is available, building durable per-Account history (consequence of C-2). _(Capability requirement; mechanism is design.)_
...
- **FR-30** ⟦OPEN-4⟧ The system _may_ offer an option to exclude recent past winners for a configurable period (candidate future Rule).
...
- **FR-31 (8a)** _Base entry_ — granted to every eligible Entity.
- **FR-32 (8b)** _No missed regular-war attacks_ — Entity missed zero attacks across the regular wars in scope.
- **FR-33 (8c)** _Participated in CWL_ — Entity was fielded in ≥1 round of the selected CWL season.
- **FR-34 (8d)** _No missed CWL attacks_ — Entity missed zero attacks in the rounds it was fielded (selected season).
- **FR-35 (8e)** _Maximum CWL stars_ — Entity earned 3 stars on every fielded attack in the selected season.
...
- **FR-45** _(Optional)_ The same ranking engine may also rank regular wars over a date range.
